...
class RegistrationController extends Controller
{
    ...
    {
        $customerType = request()->input('customer_type', 'normal');
        ...
        if ($customerType === 'trade') {
            $data = array_merge($registrationRequest->only([
                ...
                'customer_group_id'         => $this->customerGroupRepository->findOneWhere(['code' => 'wholesale'])->id,
            ...
            // Normal customer
            $data = array_merge($registrationRequest->only(['first_name', 'last_name', 'email', 'is_subscribed']), [
                'password'                  => bcrypt(request()->input('password')),
                'api_token'                 => Str::random(80),
                'is_verified'               => ! core()->getConfigData('customer.settings.email.verification'),
        ...
        if ($customerType === 'trade') {
            session()->flash('success', trans('shop::app.customers.signup-form.success-pending-approval'));

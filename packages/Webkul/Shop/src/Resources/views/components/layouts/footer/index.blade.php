...
<footer style="background:rgba(26,26,26,0.6);backdrop-filter:blur(10px);-webkit-backdrop-filter:blur(10px);color:#fff;font-family:inherit;padding:50px 20px 20px 20px;">
    <div style="display:flex;flex-wrap:wrap;gap:40px;justify-content:space-between;max-width:1400px;margin:0 auto;">
        ...
        <div style="flex:1;min-width:220px;">
            ...
            <p style="margin-top:15px;line-height:1.6;opacity:0.9;">{!! $footer?->about_text !!}</p>

            <div style="margin-top:15px;display:flex;gap:15px;flex-wrap:wrap;">
                @if($footer?->facebook)
                    <a href="{{ $footer->facebook }}" target="_blank" style="color:#fff;font-size:20px;transition:0.3s;">
                        <!-- Facebook SVG -->
                ...
                @if($footer?->instagram)
                    <a href="{{ $footer->instagram }}" target="_blank" style="color:#fff;font-size:20px;transition:0.3s;">
                        <!-- Instagram SVG -->
                        <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" fill="currentColor" viewBox="0 0 24 24">
                ...
                @if($footer?->twitter)
                    <a href="{{ $footer->twitter }}" target="_blank" style="color:#fff;font-size:20px;transition:0.3s;">
                        <!-- X (Twitter) SVG -->
                        <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" fill="currentColor" viewBox="0 0 24 24">
        ...
        <div style="flex:1;min-width:180px;">
            ...
            @if (!empty($footer?->help_links))
                <ul style="list-style:none;padding:0;margin:0;">
                    @foreach ($footer->help_links as $link)
                        <li style="margin-bottom:8px;">
                            <a href="{{ $link['url'] }}" style="color:#fff;text-decoration:none;opacity:0.85;">{{ $link['title'] }}</a>
                        </li>
                    @endforeach
                </ul>
            @endif
        ...
        <div style="flex:1;min-width:220px;">
            ...
            @if (!empty($footer?->important_links))
                <ul style="list-style:none;padding:0;margin:0;">
        ...
        <div style="flex:1;min-width:220px;">
            ...
            <p style="margin-bottom:8px;opacity:0.9;">{{ $footer?->contact_address }}</p>
            <p style="margin-bottom:8px;opacity:0.9;"><a href="tel:{{ $footer?->contact_phone }}" style="color:#fff;text-decoration:none;">{{ $footer?->contact_phone }}</a></p>
            <p style="margin-bottom:8px;opacity:0.9;"><a href="mailto:{{ $footer?->contact_email }}" style="color:#fff;text-decoration:none;">{{ $footer?->contact_email }}</a></p>
        </div>
    </div>
    ...
    <div style="border-top:1px solid rgba(255,255,255,0.15);margin-top:40px;padding-top:20px;display:flex;flex-wrap:wrap;justify-content:space-between;align-items:center;gap:15px;font-size:14px;opacity:0.85;text-align:center;">
        ...
        <div style="flex:1;">{{ $footer?->copyright_text }}</div>
